...
SELECTED_ITEMS=$(
    find /home/$1 -mindepth 1 -maxdepth 5 | fzf --multi --preview 'du -sh {}' \
        --bind "tab:execute-silent(
        grep -Fxq {} $SELECTED && sed -i '\|^{}$|d' $SELECTED || echo {} >> $SELECTED;
        xargs -d '\n' du -ch 2>/dev/null < ./fzf_selected | grep total$ | sed 's/total\s*/<-Total size /' > $SIZE
    )+toggle" \
...
if [[ -z "$SELECTED_ITEMS" ]]; then
    ...
    TOTAL_SIZE=$(du -ch $(xargs -0 <<<"${SELECTED_ITEMS[@]}") 2>/dev/null | grep "total$" | awk '{print $1}')
...
if [[ "$choice" == "y" || "$choice" == "Y" ]]; then
    ...
    find /home/$1 -type d -print0 | fzf --read0 --print0 --multi > sync_dirs.lst
else
...
cp /home/$1/.local/share/kxmlgui5/dolphin/dolphinui.rc ./backup/dolphin
cp /home/$1/.config/dolphinrc ./backup/dolphin/
mkdir -p ./backup/zsh/
cp /home/$1/.zshrc ./backup/zsh/
...
SCRIPT_DIR="sysgen"
mkdir -p "$SCRIPT_DIR"
cp -r ./*.sh utils/ *.conf *.lst "$SCRIPT_DIR"
...
# Format the storage partition as Btrfs
echo "Formatting "${USB_DEVICE}3" as Btrfs (STORAGE)..."
